...
1. **슬라이더 자체의 값(연도)은 연속적**이다. `-3000` ~ `1920` 사이 모든 정수 연도를
   ...
   표시도 실시간 갱신).
2. **지도 렌더링에 쓰는 GeoJSON만 "해당 연도 이하의 최신 스냅샷(floor)"으로 스냅**한다.
   - `snapToFloorSnapshot(year, SNAPSHOT_YEARS)` 유틸: 이진 탐색으로 O(log n) 처리.
   - 예: 슬라이더가 `-650`이면 `-700`을, `251`이면 `200`을 선택.
   - 가장 가까운 스냅샷(nearest)이 아니라 floor를 쓰는 이유: 미래 스냅샷으로 앞당겨
     붙이면 아직 일어나지 않은 역사가 보인다 (예: 251년에 300년 스냅샷의 사두정치
     4분할 로마가 표시됨). "그 시점까지 확정된 지도"를 보여주는 것이 역사적으로 자연스럽다.
   - 첫 스냅샷(`-3000`)보다 이른 연도는 첫 스냅샷으로 클램프.
